...
```
src/
├── Host/ModularAPITemplate.Host/          → Projeto Web (entry point)
├── SharedKernel/ModularAPITemplate.SharedKernel/  → Abstrações e infra compartilhada
└── Modules/
...
### Passos pós-criação:
1. Adicione os projetos criados (src e tests) à solution (`.slnx`).
...
7. Adicione a connection string no `appsettings.json`.
